...
import { getCountryList, createCountry, updateCountry, deleteCountry } from '../api/CountryApi';
...
export default function CountryProvider({ children }: { children: ReactNode }) {
  ...
  const createCountryData = (data: Record<string, any>) => createMutation.mutateAsync(data);
  const updateCountryData = (uuid: string, data: Record<string, any>) => updateMutation.mutateAsync({ uuid, data });
