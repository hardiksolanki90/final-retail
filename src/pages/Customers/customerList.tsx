...
export function CustomerList() {
  ...
  const {
    ...
    handleBulkAction,
    refetchCustomers,
    isCustomerModalVisible: isAddOpen,
    setIsCustomerModalVisible: setIsAddOpen,
  } = useCustomer();

  const customers: Customer[] = Array.isArray(customerData?.data?.customers)
    ? customerData.data.customers
    : Array.isArray(customerData?.data?.data)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pb-0 pt-6">
        <div>
          ...
          <p className="text-[var(--text-secondary)] mt-1">Manage your customer database</p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-color)]">
                ...
                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Shop Name</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[var(--bg-card)] rounded-lg shadow-xl w-full max-w-lg mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)]">
              <h2 className="text-xl font-semibold text-[var(--text-primary)]">Export Customers</h2>
