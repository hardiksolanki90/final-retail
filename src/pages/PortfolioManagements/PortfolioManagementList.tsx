...
import { PortfolioManagementAdd } from './PortfolioManagementAdd';
import { Pagination } from '../../components/ui/Pagination';

const portfolioManagementsData = [
  { id: 1, portfolioName: 'Q1 Portfolio', salesman: 'John Doe', customer: 'Acme Corp', targetRevenue: 50000, actualRevenue: 35000, status: 'Active' },
  { id: 2, portfolioName: 'Premium Clients', salesman: 'Jane Smith', customer: 'Global Trade', targetRevenue: 75000, actualRevenue: 60000, status: 'Active' },
  { id: 3, portfolioName: 'New Accounts', salesman: 'Bob Johnson', customer: 'Tech Solutions', targetRevenue: 25000, actualRevenue: 28000, status: 'Completed' },
];
...
export function PortfolioManagementList() {
  ...
  const [columns, setColumns] = useState<Column[]>([
    { key: 'portfolioName', label: 'Portfolio Name', visible: true },
    { key: 'salesman', label: 'Salesman', visible: true },
    { key: 'customer', label: 'Customer', visible: true },
    { key: 'targetRevenue', label: 'Target Revenue', visible: true },
    { key: 'actualRevenue', label: 'Actual Revenue', visible: true },
    { key: 'status', label: 'Status', visible: true },
  ]);

  // Filter state
  const [filterOpen, setFilterOpen] = useState(false);
  const [filterDraft, setFilterDraft] = useState({ portfolioName: '', salesman: '', customer: '', targetRevenue: '', actualRevenue: '', status: '' });
  const [appliedFilter, setAppliedFilter] = useState({ portfolioName: '', salesman: '', customer: '', targetRevenue: '', actualRevenue: '', status: '' });

  const totalPages = Math.ceil(portfolioManagementsData.length / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = startIndex + rowsPerPage;

  // Apply filters
  const filteredData = portfolioManagementsData.filter(c =>
    (!appliedFilter.portfolioName || String(c.portfolioName ?? '').toLowerCase().includes(appliedFilter.portfolioName.toLowerCase())) &&
    ...
    (!appliedFilter.targetRevenue || String(c.targetRevenue ?? '').toLowerCase().includes(appliedFilter.targetRevenue.toLowerCase())) &&
    (!appliedFilter.actualRevenue || String(c.actualRevenue ?? '').toLowerCase().includes(appliedFilter.actualRevenue.toLowerCase())) &&
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pl-6 pb-0 pt-6">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-primary)]">Portfolio Management</h1>
          <p className="text-[var(--text-secondary)] mt-1">Manage sales portfolios</p>
        </div>
        ...
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex items-end gap-2 pb-0.5">
              ...
              <button
                onClick={() => {
                  const empty = { portfolioName: '', salesman: '', customer: '', targetRevenue: '', actualRevenue: '', status: '' };
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] overflow-hidden transition-theme">
        ...
        <Pagination currentPage={currentPage} totalPages={totalPages} total={portfolioManagementsData.length} perPage={rowsPerPage} onPageChange={setCurrentPage} onPerPageChange={setRowsPerPage} />
      </div>
    </div>
  );
}
