...
export function InvoiceList() {
  const {
    ...
    handleDeleteWithConfirmation, handleBulkAction, handleRowClick, refetchInvoices,
  } = useInvoice();

  const invoices: Invoice[] = Array.isArray(invoiceData?.invoices)
    ? invoiceData.invoices
    : Array.isArray(invoiceData?.data?.data)
  ...
  const totalPages = invoiceData?.lastPage ?? (Math.ceil(total / perPage) || 1);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pb-0 pt-6">
        <div>
          ...
          <p className="text-[var(--text-secondary)] mt-1">Manage your billing invoices</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[var(--border-color)]">
              {!isLoading && invoices.length === 0 && (<tr><td colSpan={8} className="px-4 py-12 text-center text-[var(--text-muted)]">No invoices found.</td></tr>)}
