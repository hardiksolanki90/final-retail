...
const routeItemGroupingsData = [
  { id: 1, groupName: 'Fast Moving Items', route: 'Route A', itemCount: 25, priority: 1, status: 'Active' },
  { id: 2, groupName: 'Seasonal Products', route: 'Route B', itemCount: 15, priority: 2, status: 'Active' },
  { id: 3, groupName: 'Premium Items', route: 'Route A', itemCount: 10, priority: 3, status: 'Active' },
  { id: 4, groupName: 'Promotional Pack', route: 'Route C', itemCount: 20, priority: 1, status: 'Active' },
  { id: 5, groupName: 'Basic Necessities', route: 'Route D', itemCount: 30, priority: 1, status: 'Active' },
];
...
export function RouteItemGroupingList() {
  ...
  const [filterDraft, setFilterDraft] = useState({ groupName: '', route: '', itemCount: '', priority: '', status: '' });
  const [appliedFilter, setAppliedFilter] = useState({ groupName: '', route: '', itemCount: '', priority: '', status: '' });

  const totalPages = Math.ceil(routeItemGroupingsData.length / rowsPerPage);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex items-end gap-2 pb-0.5">
              ...
              <button
                onClick={() => {
                  const empty = { groupName: '', route: '', itemCount: '', priority: '', status: '' };
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] overflow-hidden transition-theme">
        ...
        <Pagination currentPage={currentPage} totalPages={totalPages} total={routeItemGroupingsData.length} perPage={rowsPerPage} onPageChange={setCurrentPage} onPerPageChange={setRowsPerPage} />
      </div>
    </div>
  );
}
