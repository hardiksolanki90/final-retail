  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] overflow-hidden transition-theme">
                ...
                >
                  ...
                  {visibleColumns.map((column) => (
                    <td key={column.key} className="px-4 py-3 text-sm text-[var(--text-primary)]">
                      {column.key === 'price'
                        ? `$${Number(item[column.key as keyof typeof item]).toFixed(2)}`
        ...
        <Pagination currentPage={currentPage} totalPages={totalPages} total={competitorInfosData.length} perPage={rowsPerPage} onPageChange={setCurrentPage} onPerPageChange={setRowsPerPage} />
      </div>
    </div>
  );
}
