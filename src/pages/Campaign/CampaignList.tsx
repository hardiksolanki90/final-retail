...
const campaignsData = [
  { id: 1, code: 'CAM001', name: 'Summer Sale 2024', type: 'Seasonal', startDate: '2024-06-01', endDate: '2024-08-31', budget: 50000, status: 'Active' },
  { id: 2, code: 'CAM002', name: 'New Product Launch', type: 'Launch', startDate: '2024-07-15', endDate: '2024-08-15', budget: 75000, status: 'Active' },
  { id: 3, code: 'CAM003', name: 'Holiday Promo', type: 'Holiday', startDate: '2024-12-01', endDate: '2024-12-31', budget: 100000, status: 'Scheduled' },
  { id: 4, code: 'CAM004', name: 'Back to School', type: 'Seasonal', startDate: '2024-08-15', endDate: '2024-09-30', budget: 35000, status: 'Scheduled' },
  { id: 5, code: 'CAM005', name: 'Spring Collection', type: 'Launch', startDate: '2024-03-01', endDate: '2024-05-31', budget: 45000, status: 'Completed' },
  { id: 6, code: 'CAM006', name: 'Flash Sale Week', type: 'Promotional', startDate: '2024-07-01', endDate: '2024-07-07', budget: 20000, status: 'Active' },
];
...
export function CampaignList() {
  ...
  const [filterDraft, setFilterDraft] = useState({ code: '', name: '', type: '', startDate: '', endDate: '', budget: '' });
  const [appliedFilter, setAppliedFilter] = useState({ code: '', name: '', type: '', startDate: '', endDate: '', budget: '' });
  ...
  const totalPages = Math.ceil(campaignsData.length / rowsPerPage);
  ...
  const filteredData = campaignsData.filter(c =>
    ...
    (!appliedFilter.budget || String(c.budget ?? '').toLowerCase().includes(appliedFilter.budget.toLowerCase()))
  );
  ...
  const getStatusBadge = (status: string) => {
    ...
    switch (status) {
      case 'Active': return `${baseClasses} `;
      case 'Scheduled': return `${baseClasses} `;
      case 'Completed': return `${baseClasses} `;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pl-6 pb-0 pt-6">
        <div>
          ...
          <p className="text-[var(--text-secondary)] mt-1">Manage marketing campaigns</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {selectedRows.length > 0 && (
            <div className="relative" ref={bulkActionRef}>
              <button onClick={() => setBulkActionOpen(!bulkActionOpen)} className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border transition-colors bg-[var(--bg-card)] border-[var(--border-color)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary)]">Bulk Action<span className="ml-1 px-1.5 py-0.5 text-xs bg-primary-100 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 rounded">{selectedRows.length}</span><ChevronDown className="w-4 h-4" /></button>
        ...
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex items-end gap-2 pb-0.5">
              ...
              <button
                onClick={() => {
                  const empty = { code: '', name: '', type: '', startDate: '', endDate: '', budget: '' };
