...
import {
  Filter,
  Plus, RefreshCw, Columns3, Download, Upload, ChevronDown, Check, X, Menu
} from 'lucide-react';
...
// Sample data
const replacementData: MerchandiserReplacement[] = [
  {
    id: 1,
    originalMerchandiserId: '1',
    replacementMerchandiserId: '3',
  ...
  {
    id: 2,
    originalMerchandiserId: '2',
    replacementMerchandiserId: '4',
...
export function MerchandiserReplacementList() {
  ...
  const [columns, setColumns] = useState<Column[]>([
    { key: 'originalMerchandiserId', label: 'Original Merchandiser', visible: true },
    { key: 'replacementMerchandiserId', label: 'Replacement', visible: true },
  ...
  const [filterDraft, setFilterDraft] = useState({ originalMerchandiserId: '', replacementMerchandiserId: '', startDate: '', endDate: '', status: '' });
  const [appliedFilter, setAppliedFilter] = useState({ originalMerchandiserId: '', replacementMerchandiserId: '', startDate: '', endDate: '', status: '' });
  ...
  const filteredData = replacementData.filter(c =>
    (!appliedFilter.originalMerchandiserId || String(c.originalMerchandiserId ?? '').toLowerCase().includes(appliedFilter.originalMerchandiserId.toLowerCase())) &&
    (!appliedFilter.replacementMerchandiserId || String(c.replacementMerchandiserId ?? '').toLowerCase().includes(appliedFilter.replacementMerchandiserId.toLowerCase())) &&
  ...
  const handleExportSubmit = () => {
    console.log('Exporting merchandiser replacements...', { type: exportType, fromDate: exportFromDate, toDate: exportToDate, format: exportFormat });
  ...
  const handleImport = () => console.log('Importing merchandiser replacements...');
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--text-secondary)] mt-1">
              Manage merchandiser replacements and substitutions
            </p>
          </div>
        </div>
      ...
      {filterOpen && (
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            {([
              { key: 'originalMerchandiserId', label: 'Original Merchandiser Id' },
              { key: 'replacementMerchandiserId', label: 'Replacement Merchandiser Id' },
            ...
            <div className="flex items-end gap-2 pb-0.5">
              ...
              <button
                onClick={() => {
                  const empty = { originalMerchandiserId: '', replacementMerchandiserId: '', startDate: '', endDate: '', status: '' };
              ...
              {currentData.map((item) => (
                ...
                >
                  ...
                  {visibleColumns.map((column) => (
                    ...
                    ) : column.key === 'originalMerchandiserId' || column.key === 'replacementMerchandiserId' ? (
                      <span>Merchandiser {item[column.key as keyof MerchandiserReplacement]}</span>
                    ) : (
                      item[column.key as keyof MerchandiserReplacement]
                    )}
                  </td>
                  ))}
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[var(--bg-card)] rounded-lg shadow-xl w-full max-w-lg mx-4 text-[var(--text-primary)]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)]">
              <h2 className="text-xl font-semibold">Export Merchandiser Replacements</h2>
              <button
            ...
            <div className="px-6 py-4 space-y-6">
              <p className="text-[var(--text-secondary)]">Export merchandiser replacement data in CSV or XLS format.</p>
      ...
      <MerchandiserReplacementAdd
        ...
        onSubmit={(data) => {
          console.log('Create Replacement:', data);
          setIsAddOpen(false);
        }}
      />
    </div>
