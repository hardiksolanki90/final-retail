...
import {
  Filter,
  Plus, Building, Columns3, Download, Upload, ChevronDown, Check, X, Menu
} from 'lucide-react';
...
const warehousesData = [
  { id: 1, code: 'WH001', name: 'Main Warehouse', address: 'Dubai Industrial Area 1', city: 'Dubai', capacity: 5000 },
  { id: 2, code: 'WH002', name: 'Secondary Warehouse', address: 'Sharjah Industrial Area 3', city: 'Sharjah', capacity: 3000 },
];
...
export function WarehouseList() {
  ...
  const [filterDraft, setFilterDraft] = useState({ code: '', name: '', address: '', city: '', capacity: '' });
  const [appliedFilter, setAppliedFilter] = useState({ code: '', name: '', address: '', city: '', capacity: '' });
  ...
  const totalPages = Math.ceil(warehousesData.length / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = startIndex + rowsPerPage;

  // Apply filters
  const filteredData = warehousesData.filter(c =>
    ...
    (!appliedFilter.address || String(c.address ?? '').toLowerCase().includes(appliedFilter.address.toLowerCase())) &&
    (!appliedFilter.city || String(c.city ?? '').toLowerCase().includes(appliedFilter.city.toLowerCase())) &&
    (!appliedFilter.capacity || String(c.capacity ?? '').toLowerCase().includes(appliedFilter.capacity.toLowerCase()))
  );
  ...
  const handleExportSubmit = () => {
    console.log('Exporting warehouses...', { type: exportType, fromDate: exportFromDate, toDate: exportToDate, format: exportFormat });
  ...
  const handleImport = () => console.log('Importing warehouses...');
        ...
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex items-end gap-2 pb-0.5">
              ...
              <button
                onClick={() => {
                  const empty = { code: '', name: '', address: '', city: '', capacity: '' };
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] overflow-hidden transition-theme">
              ...
              {currentData.map((warehouse) => (
                <tr
                  ...
                  className={`hover:bg-[var(--bg-secondary)] transition-colors ${
                    selectedRows.includes(warehouse.id) ? 'bg-primary-50 dark:bg-primary-900/10' : ''
                  }`}
                >
                  <td className="px-4 py-3">
                    <input
                      ...
                      onChange={() => handleSelectRow(warehouse.id)}
                      className="w-4 h-4 rounded border-[var(--border-color)] text-primary-600 focus:ring-primary-500"
                    />
                  </td>
                  {visibleColumns.map((column) => (
                    <td key={column.key} className="px-4 py-3 text-sm text-[var(--text-primary)]">
                      {warehouse[column.key as keyof typeof warehouse]}
                    </td>
                  ))}
                </tr>
              ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 border-t border-[var(--border-color)]">
          <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
            ...
            <span className="ml-4">
              {startIndex + 1}-{Math.min(endIndex, warehousesData.length)} of {warehousesData.length}
            </span>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[var(--bg-card)] rounded-lg shadow-xl w-full max-w-lg mx-4 text-[var(--text-primary)]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)]">
              <h2 className="text-xl font-semibold">Export Warehouse</h2>
              <button
            ...
            <div className="px-6 py-4 space-y-6">
              <p className="text-[var(--text-secondary)]">Export warehouse data in CSV or XLS format.</p>
