  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            <div className="flex flex-col gap-1 flex-1 min-w-[200px]">
              ...
              <input type="text" value={searchDraft} onChange={e => setSearchDraft(e.target.value)} onKeyDown={e => e.key === 'Enter' && applySearch()} placeholder="Search by name…" className="px-3 py-2 text-sm rounded-lg border border-[var(--border-color)] bg-[var(--bg-secondary)] text-[var(--text-primary)] placeholder-[var(--text-muted)] focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500" />
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[var(--border-color)]">
              {!isLoading && taxes.length === 0 && (<tr><td colSpan={6} className="px-4 py-12 text-center text-[var(--text-muted)]">No taxes found.</td></tr>)}
              {taxes.map((tax: any) => {
                ...
                return (
                  <tr key={id} className="group hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all duration-200 cursor-pointer">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm text-[var(--text-primary)]">{tax?.rate != null ? `${tax.rate}%` : '—'}</span></td>
      ...
      <TaxesAdd
        ...
        onSubmit={async (data) => {
          if (editingItem?.uuid) {
            await updateTaxData(editingItem.uuid, data);
          } else {
            await createTaxData(data);
          }
          handleSaved();
        }}
      />
