...
import { Filter, Plus, Building2, Columns3, Download, Upload, ChevronDown, Check, Menu, Pencil, Trash2 } from 'lucide-react';
...
export function BankList() {
  const { data: banks, meta, isLoading, error, currentPage, setCurrentPage, perPage, setPerPage, searchTerm, setSearchTerm, selectedRowKeys, setSelectedRowKeys, handleDeleteWithConfirmation, addDrawerOpen, setAddDrawerOpen, editingItem, setEditingItem, createBankData, updateBankData } = useBank();
  ...
  const [columns, setColumns] = useState<Column[]>([
    { key: 'bankCode', label: 'Bank Code', visible: true },
    { key: 'bankName', label: 'Bank Name', visible: true },
    { key: 'accountNumber', label: 'Account Number', visible: true },
    { key: 'routingCode', label: 'Routing Code', visible: true },
    { key: 'bankAddress', label: 'Bank Address', visible: true },
  ...
  const getCellValue = (item: any, key: string) => {
    switch (key) {
      ...
      case 'bankName': return item.bankName ?? '';
      case 'accountNumber': return item.accountNumber ?? '—';
      case 'routingCode': return item.iban || item.ifscCode || item.routingNumber || item.sortCode || item.swiftCode || '—';
      case 'bankAddress': return item.bankAddress ?? '—';
      case 'status': return item.status ? 'Active' : 'Inactive';
      default: return '';
    }
  };
  const allSelected = banks.length > 0 && selectedRowKeys.length === banks.length;
  const handleSelectAll = () => setSelectedRowKeys(allSelected ? [] : banks.map((c: any) => c.uuid ?? String(c.id)));
  ...
  const handleBankEvent = async (event: any) => {
    if (event.eventType === 'BankUpdated' && editingItem) {
      await updateBankData(editingItem.uuid, event.bank);
    } else {
      await createBankData(event.bank);
    }
    handleSaved();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pb-0 pt-6">
        <div className="flex items-center gap-3"><Building2 className="w-6 h-6 text-[var(--text-primary)]" /><div><h1 className="text-2xl font-bold text-[var(--text-primary)]">Banks</h1><p className="text-[var(--text-secondary)] mt-1">Manage bank accounts</p></div></div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[var(--border-color)]">
              {!isLoading && banks.length === 0 && (<tr><td colSpan={visibleColumns.length + 2} className="px-4 py-12 text-center text-[var(--text-muted)]">No banks found.</td></tr>)}
              {banks.map((item: any) => {
      ...
      <BankAdd isOpen={addDrawerOpen} onClose={handleDrawerClose} data={{ initialData: editingItem }} onEvent={handleBankEvent} />
    </div>
  );
}
