...
interface ReasonAddProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: ReasonFormData) => void | Promise<void>;
...
export function ReasonAdd({
  ...
  const onFormSubmit = async (data: ReasonFormData) => {
    try {
      await onSubmit(data);
      onClose();
    } catch (error: any) {
  ...
  const footerContent = (
    <div className="flex items-center justify-end gap-3">
      ...
      <SaveButton type="submit" form="reason-form" disabled={isSubmitting}>
