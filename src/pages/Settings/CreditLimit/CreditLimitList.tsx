...
import { Plus, CreditCard, Columns3, ChevronDown, Check, Menu, Download, Upload, Pencil, Trash2 } from 'lucide-react';
import { CreditLimitAdd } from './CreditLimitAdd';
import { useCreditLimit } from '../../../providers/CreditLimitProvider';
import { createCreditLimit, updateCreditLimit } from '../../../api/CreditLimitApi';
...
export default function CreditLimitList() {
  ...
  const handleEditClick = (item: CreditLimit) => { setEditingItem(item); setAddDrawerOpen(true); };
  const handleDrawerClose = () => { setAddDrawerOpen(false); setEditingItem(null); };

  const handleSave = async (data: CreditLimitFormData) => {
    if (editingItem) {
      await updateCreditLimit(editingItem.uuid, data);
    } else {
      await createCreditLimit(data);
    }
    handleDrawerClose();
    refetch();
  };
