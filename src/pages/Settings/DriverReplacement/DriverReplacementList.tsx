...
export function DriverReplacementList() {
  const {
    ...
    selectedRowKeys, setSelectedRowKeys, handleDeleteWithConfirmation,
    addDrawerOpen, setAddDrawerOpen, editingItem, setEditingItem,
    salesmanOptions, vanOptions, reasonOptions, handleCreate, handleUpdate,
  } = useDriverReplacement();
  ...
  const [columns, setColumns] = useState<Column[]>([
    ...
    { key: 'oldVan', label: 'Old Van', visible: true },
    { key: 'newVan', label: 'New Van', visible: true },
  ...
  const getCellValue = (item: any, key: string) => {
    if (key === 'oldSalesman') return item.oldSalesman?.name || '-';
    if (key === 'newSalesman') return item.newSalesman?.name || '-';
    if (key === 'oldVan') return item.oldVan ? `${item.oldVan.vanCode || ''} - ${item.oldVan.plateNumber || ''}`.replace(/^ - | - $/g, '') || '-' : '-';
    if (key === 'newVan') return item.newVan ? `${item.newVan.vanCode || ''} - ${item.newVan.plateNumber || ''}`.replace(/^ - | - $/g, '') || '-' : '-';
    if (key === 'reason') return item.reason?.name || '-';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pb-0 pt-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[var(--text-secondary)] mt-1">Manage driver and van swaps</p>
          </div>
        </div>
      ...
      {filterOpen && (
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <p className="text-sm text-[var(--text-muted)]">Filter options can be added here (old/new salesman, reason, date).</p>
        </div>
      )}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-color)]">
                ...
                {visibleColumns.map(col => <th key={col.key} className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">{col.label}</th>)}
      ...
      <DriverReplacementAdd
        ...
        onSubmit={async (data) => {
          if (editingItem) await handleUpdate(editingItem.uuid, data);
          else await handleCreate(data);
          handleSaved();
        }}
        initialData={editingItem ? {
          oldSalesmanId: editingItem.oldSalesmanId ?? '',
          newSalesmanId: editingItem.newSalesmanId ?? '',
          oldVanId: editingItem.oldVanId ?? '',
          newVanId: editingItem.newVanId ?? '',
          reasonId: editingItem.reasonId ?? '',
