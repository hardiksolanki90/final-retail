...
import {
  Filter,
  Plus, Barcode, Columns3, Download, Upload, ChevronDown, Check, X, Menu
} from 'lucide-react';
import { OutletProductCodeAdd } from './OutletProductCodeAdd';

interface Column { key: string; label: string; visible: boolean; }

// Sample outlet product code data
const outletProductCodesData = [
  { id: 1, code: 'OPC001', productName: 'Coca Cola 330ml', outletName: 'Super Store', customerName: 'ABC Trading' },
  { id: 2, code: 'OPC002', productName: 'Pepsi 330ml', outletName: 'Mini Mart', customerName: 'XYZ Distributors' },
];
...
export function OutletProductCodeList() {
  ...
  const [columns, setColumns] = useState<Column[]>([
    ...
    { key: 'outletName', label: 'Outlet Name', visible: true },
    { key: 'customerName', label: 'Customer Name', visible: true },
  ]);

  // Filter state
  const [filterOpen, setFilterOpen] = useState(false);
  const [filterDraft, setFilterDraft] = useState({ code: '', productName: '', outletName: '', customerName: '' });
  const [appliedFilter, setAppliedFilter] = useState({ code: '', productName: '', outletName: '', customerName: '' });
  ...
  const totalPages = Math.ceil(outletProductCodesData.length / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = startIndex + rowsPerPage;

  // Apply filters
  const filteredData = outletProductCodesData.filter(c =>
    ...
    (!appliedFilter.outletName || String(c.outletName ?? '').toLowerCase().includes(appliedFilter.outletName.toLowerCase())) &&
  ...
  const handleExportSubmit = () => {
    console.log('Exporting outlet product codes...', { type: exportType, fromDate: exportFromDate, toDate: exportToDate, format: exportFormat });
  ...
  const handleImport = () => console.log('Importing outlet product codes...');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pl-6 pb-0 pt-6">
        <div className="flex items-center gap-3">
          <Barcode className="w-6 h-6 text-[var(--text-primary)]" />
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Outlet Product Code</h1>
            <p className="text-[var(--text-secondary)] mt-1">
              Manage outlet product codes
            </p>
          </div>
        </div>
        ...
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex items-end gap-2 pb-0.5">
              ...
              <button
                onClick={() => {
                  const empty = { code: '', productName: '', outletName: '', customerName: '' };
      ...
      <div className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-color)] overflow-hidden transition-theme">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 border-t border-[var(--border-color)]">
          <div className="flex items-center gap-2 text-sm text-[var(--text-secondary)]">
            ...
            <span className="ml-4">
              {startIndex + 1}-{Math.min(endIndex, outletProductCodesData.length)} of {outletProductCodesData.length}
            </span>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[var(--bg-card)] rounded-lg shadow-xl w-full max-w-lg mx-4 text-[var(--text-primary)]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)]">
              <h2 className="text-xl font-semibold">Export Outlet Product Codes</h2>
              <button
      ...
      <OutletProductCodeAdd
        ...
        onSubmit={(data) => {
          console.log('Outlet product code data submitted:', data);
