...
export function BeatList() {
  ...
  const activeFilterCount = [searchTerm, areaFilter].filter(Boolean).length;

  const [columns, setColumns] = useState<Column[]>([
    { key: 'beatCode', label: 'Beat Code', visible: true },
    { key: 'beatName', label: 'Beat Name', visible: true },
  ...
  const isBeatCodeVisible = isColumnVisible('beatCode');
  const isBeatNameVisible = isColumnVisible('beatName');
  const isAreaVisible = isColumnVisible('area');
  const isStatusVisible = isColumnVisible('status');
  const visibleColCount =
    2 +
    (isBeatCodeVisible ? 1 : 0) +
    (isBeatNameVisible ? 1 : 0) +
    (isAreaVisible ? 1 : 0) +
    (isStatusVisible ? 1 : 0);

  const allSelected = beats.length > 0 && selectedRowKeys.length === beats.length;
  const handleSelectAll = () => setSelectedRowKeys(allSelected ? [] : beats.map((c: any) => c.uuid ?? String(c.id)));
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pb-0 pt-6">
        <div className="flex items-center gap-3">
          <Navigation className="w-6 h-6 text-[var(--text-primary)]" />
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Beats</h1>
            <p className="text-[var(--text-secondary)] mt-1">Manage beats</p>
          </div>
        </div>
                    ...
                    {isAreaVisible && (
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span className="text-sm text-[var(--text-secondary)]">
                          {item?.area ? (item.area.code ? `${item.area.code} - ${item.area.name}` : (item.area.name || item.area.areaName)) : '—'}
                        </span>
                      </td>
                    )}
                    {isStatusVisible && (
                      <td className="px-4 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${item?.status ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'}`}
                        >
