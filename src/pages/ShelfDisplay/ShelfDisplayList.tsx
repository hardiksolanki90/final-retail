...
const shelfDisplayData = [
  { id: 1, code: 'SD001', name: 'Premium End Cap', location: 'Aisle 1', type: 'End Cap', products: 12, status: 'Active' },
  { id: 2, code: 'SD002', name: 'Seasonal Display', location: 'Entrance', type: 'Free Standing', products: 8, status: 'Active' },
  { id: 3, code: 'SD003', name: 'Checkout Rack', location: 'Checkout', type: 'Counter', products: 20, status: 'Active' },
  { id: 4, code: 'SD004', name: 'Wall Unit A', location: 'Wall Section', type: 'Wall Mount', products: 15, status: 'Inactive' },
  { id: 5, code: 'SD005', name: 'Island Display', location: 'Center', type: 'Island', products: 24, status: 'Active' },
];

interface Column { key: string; label: string; visible: boolean; }

export function ShelfDisplayList() {
  ...
  const [filterDraft, setFilterDraft] = useState({ code: '', name: '', location: '', type: '', products: '', status: '' });
  const [appliedFilter, setAppliedFilter] = useState({ code: '', name: '', location: '', type: '', products: '', status: '' });
  ...
  const totalPages = Math.ceil(shelfDisplayData.length / rowsPerPage);
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = startIndex + rowsPerPage;

  // Apply filters
  const filteredData = shelfDisplayData.filter(c =>
    ...
    (!appliedFilter.products || String(c.products ?? '').toLowerCase().includes(appliedFilter.products.toLowerCase())) &&
  ...
  const getStatusBadge = (status: string) => { const baseClasses = 'px-2 py-1 text-xs font-medium '; return status === 'Active' ? `${baseClasses} ` : `${baseClasses} `; };

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pl-6 pb-0 pt-6">
        <div><h1 className="text-2xl font-bold text-[var(--text-primary)]">Shelf Display</h1><p className="text-[var(--text-secondary)] mt-1">Manage shelf displays</p></div>
        ...
        <div className="mx-6 mb-2 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl px-5 py-4 shadow-sm">
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="flex items-end gap-2 pb-0.5">
              ...
              <button
                onClick={() => {
                  const empty = { code: '', name: '', location: '', type: '', products: '', status: '' };
