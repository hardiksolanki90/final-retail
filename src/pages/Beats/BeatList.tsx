...
export function BeatList() {
  ...
  const [columns, setColumns] = useState<Column[]>([
    { key: 'areaName', label: 'Area Name', visible: true },
    { key: 'nodeLevel', label: 'Level', visible: true },
  ...
  const allSelected = areas.length > 0 && selectedRowKeys.length === areas.length;
  const handleSelectAll = () => setSelectedRowKeys(allSelected ? [] : areas.map((c: any) => c.uuid ?? String(c.id)));
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pb-0 pt-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-[var(--text-primary)]">Areas</h1>
            <p className="text-[var(--text-secondary)] mt-1">Manage areas</p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={() => setFilterOpen((prev) => !prev)}
            className={`inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border transition-colors ${filterOpen || searchTerm ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-300 dark:border-primary-700 text-primary-700 dark:text-primary-300' : 'bg-[var(--bg-card)] border-[var(--border-color)] text-[var(--text-primary)] hover:bg-[var(--bg-secondary)]'}`}
          >
              ...
              {areas.map((item: any) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span className="text-sm text-[var(--text-secondary)]">{item?.nodeLevel ?? '-'}</span>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-4 py-3 border-t border-[var(--border-color)]">
          ...
          <div className="flex items-center gap-1">
            {(['First', 'Prev', 'Next', 'Last'] as const).map((label) => {
              const disabled =
                label === 'First' || label === 'Prev' ? currentPage === 1 : currentPage === totalPages || totalPages === 0;
              const onClick = () => {
      ...
      <BeatAdd isOpen={addDrawerOpen} onClose={handleDrawerClose} onSubmit={() => handleSaved()} editData={editingItem} />
    </div>
  );
}
