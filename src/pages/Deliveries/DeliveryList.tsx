...
export function DeliveryList() {
  ...
  const [columns, setColumns] = useState<Column[]>([
    { key: 'deliveryNumber', label: 'Delivery #', visible: true },
  ...
  const handleSelectAll = () => setSelectedRowKeys(allSelected ? [] : deliveries.map(d => d.uuid!));
  ...
  const bulkActions = [
    ...
    { label: 'Dispatch Selected', icon: Archive, action: () => { handleBulkAction('dispatch'); setBulkActionOpen(false); } },
    { label: 'Mark Delivered', icon: Tag, action: () => { handleBulkAction('deliver'); setBulkActionOpen(false); } },
  ];
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-6 pb-0 pt-6">
        <div>
          ...
          <p className="text-[var(--text-secondary)] mt-1">Manage your delivery schedule</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead><tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-color)]">
              ...
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Delivery #</th>
              ...
              <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Driver</th>
              <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wider text-[var(--text-muted)]">Actions</th>
            </tr></thead>
            <tbody className="divide-y divide-[var(--border-color)]">
              {!isLoading && deliveries.length === 0 && (<tr><td colSpan={8} className="px-4 py-12 text-center text-[var(--text-muted)]">No deliveries found.</td></tr>)}
              {deliveries.map((delivery: any) => (
                <tr key={delivery?.uuid} onClick={() => handleRowClick(delivery)} className="group hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all duration-200 cursor-pointer">
                  <td className="px-4 py-4 whitespace-nowrap"><input type="checkbox" checked={selectedRowKeys.includes(delivery?.uuid ?? "")} onChange={(e) => { e.stopPropagation(); handleSelectRow(delivery?.uuid ?? ""); }} onClick={e => e.stopPropagation()} className="w-4 h-4 rounded border-[var(--border-color)] text-primary-600 focus:ring-primary-500" /></td>
                  <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm font-mono text-[var(--text-secondary)]">{delivery?.delivery_number ?? delivery?.deliveryNumber ?? ''}</span></td>
                  <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm text-[var(--text-secondary)]">{delivery?.order?.order_number ?? '—'}</span></td>
                  <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm font-medium text-[var(--text-primary)]">{delivery?.customer?.name ?? delivery?.customer_name ?? '—'}</span></td>
                  <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm text-[var(--text-secondary)]">{delivery?.delivery_date ?? delivery?.date ?? delivery?.created_at ?? '—'}</span></td>
                  <td className="px-4 py-4 whitespace-nowrap"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusBadge(delivery?.status ?? '')}`}>{delivery?.status ?? '—'}</span></td>
                  <td className="px-4 py-4 whitespace-nowrap"><span className="text-sm text-[var(--text-secondary)]">{delivery?.driver?.name ?? delivery?.driver_name ?? '—'}</span></td>
                  <td className="px-4 py-4 whitespace-nowrap"><div className="flex items-center justify-end gap-2"><button className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-red-700 bg-red-50 dark:bg-red-900/30 dark:text-red-400 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/50 transition-all duration-200 group-hover:shadow-md" onClick={(e) => { e.stopPropagation(); handleDeleteWithConfirmation(delivery?.uuid ?? ""); }}><Trash2 size={14} strokeWidth={2.5} /><span>Delete</span></button></div></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {exportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[var(--bg-card)] rounded-lg shadow-xl w-full max-w-lg mx-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--border-color)]"><h2 className="text-xl font-semibold text-[var(--text-primary)]">Export Deliveries</h2><button onClick={() => setExportModalOpen(false)} className="p-1 rounded hover:bg-[var(--bg-secondary)] transition-colors"><X className="w-5 h-5 text-[var(--text-muted)]" /></button></div>
            <div className="px-6 py-4 space-y-6">
              <div className="space-y-3">{(['all', 'specific'] as const).map(t => (<label key={t} className="flex items-center gap-3 cursor-pointer"><input type="radio" name="exportType" checked={exportType === t} onChange={() => setExportType(t)} className="w-5 h-5 text-primary-600" /><span className="text-[var(--text-primary)] font-medium capitalize">{t} Deliveries</span></label>))}</div>
